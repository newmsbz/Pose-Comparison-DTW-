...
def scoring_two_vector(current_frame, ref_vectors, rendered_vectors, op_confidences, ref_confidences):  # scoring 함수
    ...
    avg_score = round(sum_score / len(ref_vectors), 2)  # 현 frame의 최종 score
...
def print_result_text(frame_num, score_list, confidence_list, fps_avg, num_sync):  # 동작 비교 결과를 텍스트로 출력하는 함수
    ...
    if num_sync * SYNC_CYCLE_PER_FRAME > frame_num / 2:
        print("체험자의 동작이 전체적으로 느립니다.")
    elif num_sync * SYNC_CYCLE_PER_FRAME > frame_num / 4:
        print("체험자의 동작이 부분적으로 느립니다.")
...
def draw_graph_fixed_result(frame_num_list, score_list, fixed_score_list, confidence_list, sync_data):
    plt.plot(frame_num_list, score_list, label='Score')
    plt.plot(frame_num_list, fixed_score_list, label='Fixed Score')
    plt.plot(frame_num_list, confidence_list, label='Confidence')
...
def sync_result_analysis(ref_frame_list, ref_vector_list, cam_frame_list, cam_vector_list, sync_data, fps_list,
    ...
    for i in range(sync_count):
        ...
        if sync_data[i][0][1] + sync_data[i][1] >= len(cam_frame_list) - 1:
            ...
            plt.legend()
            plt.show()
            score_list[sync_data[i][0][0]:sync_data[i][0][1]] = fix_avg_score
...
def main():
    ...
    real_time_graph.canvas.set_window_title('Real-Time Score & Confidence Graph')
    real_time_graph_mng = plt.get_current_fig_manager()
    real_time_graph_mng.window.setGeometry(8, 543, 1280, 480)
    ...
    if len(sync_data) >= 1:
        ...
        if slow_anal_check == '1':
            fixed_score = sync_result_analysis(ref_rendered_frames, ref_vectors, cam_frame_list, cam_vector_stack,
                                               sync_data, fps_list, score_list)
            draw_graph_fixed_result(frame_num_list, score_list, fixed_score, confidence_list, sync_data)
        else:
            pass
